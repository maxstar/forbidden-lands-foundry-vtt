...
export class ForbiddenLandsMonsterSheet extends ForbiddenLandsActorSheet {
  ...
  activateListeners(html) {
    ...
    html.find(".roll-armor").click((ev) => {
      ...
      RollDialog.prepareRollDialog(testName, 0, 0, armorValue, "", 0, 0);
    });
    html.find(".roll-attack").click((ev) => {
      const itemId = $(ev.currentTarget).data("itemId");
      const weapon = this.actor.getOwnedItem(itemId);
      let testName = weapon.name;
      RollDialog.prepareRollDialog(testName, weapon.data.data.dice, 0, 0, "", 0, weapon.data.data.damage);
    });
  }
      ...
      buttons = [
        {
          ...
          class: "custom-roll",
          icon: "fas fa-dice",
          onclick: (ev) => RollDialog.prepareRollDialog("Roll", 0, 0, 0, "", 0, 0),
        },
      ].concat(buttons);
    }
